...
public class SessionReadServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		// 		==> 세션에 저장된 모든 '세션key'값들을 구해서 Enumeration 객체로 변환한다.
		...
		if(cnt == 0) {
			out.println("<li>세션 데이터가 1도 없습니다.</li>");
		}
		out.println("</ol>");
		
		// 세션을 구분하기 위한 고유한 값
		out.println("세션 ID : " + session.getId() + "<br>");
		// 생성시간 ==> 1970년1월1일 부터 경과한 시간을 반환(밀리세컨드 단위)
		out.println("세션 생성시간 : " + session.getCreationTime() + "<br>");
		// 최근 접근시간 ==> 1970년1월1일 부터 경과한 시간을 반환(밀리세컨드 단위)
		out.println("세션 최근 접근 시간 : " + session.getLastAccessedTime() + "<br>");
		// 세션 유효시간 ==> 초 단위로 나옴
		// 세션의 최근 접근시간부터 세션 유효시간이 경과하면 세션이 자동으로 삭제됨
