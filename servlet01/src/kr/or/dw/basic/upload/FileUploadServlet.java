...
public class FileUploadServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		File fileUploadDiretory = new File(UPLOAD_DIR);
		if(!fileUploadDiretory.exists()) {
			fileUploadDiretory.mkdirs();
		}
		...
		for(Part part : req.getParts()) {
			...
			if("".equals(fileName)) continue;	// 파일 이름이 없으면 건너 뛴다.
	...
	private String getFileName(Part part) {
		 ...
		 * 특수한 경우는 Content-Disposition에 attachment 를 주는 경우로, 이 때 fileName과 함께 작성해주면
