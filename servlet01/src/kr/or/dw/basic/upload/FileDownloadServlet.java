...
public class FileDownloadServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if(file.exists()) {
			...
			// (이메일과 함께 첨부한 파일을 텍스트 문자로 전환해서 email system 으로 전달하기 위해 개발됨)
			// UUEncode 방식 : ASCII 파일만 가능
			// MIME : Binary File(2진타입)(music, movie, word files,....) -> Text File
			...
			// 멀티파트 타입 : 일반적으로 다른 MIME 타입들을 지닌 개별적인 파트들로 나누어지는 문서의 카테고리
			// ex : multipart/form-data, multipart/byteranges
			
			// 그새어 application/octet-stream은?
			// MIME의 개별 타입 중 application에 속하는 타입 8비트 단위의 Binary data라는 뜻
			// 브라우저가 보통 자동으로 실행하지 않거나 실행할지 묻기도 하는 타입이다.
			// Content-Dispostiton 헤더를 attachment 로 줌으로써 해당 데이터를 수신받은 브라우저가 저장 할 수 있도록 설정할 수 있다.
	...
	private String getFileNameEncoding(String fileName, String Browser) throws Exception{
		...
		if("MSIE".equals(Browser)) {
			// URLEncode 를 실행하면 띄어쓰기가 + 로 표시되어 다른 문자열로 바꿔준다.
			encodedFileName = URLEncoder.encode(fileName, "utf-8").replaceAll("\\+", "%20");
		}else if("Chrome".equals(Browser)) {
			StringBuffer sb = new StringBuffer();
			for (int i = 0; i < fileName.length(); i++) {
				char c = fileName.charAt(i);
				if(c > '~') {	// '~' 문자는 ASCII문자 중 제일 마지막 위치의 문자이다. ASCII 코드표에 포함 하냐 안하냐를 물어보는 조건
