...
public class ResponseTest02 extends HttpServlet {
	...
	// forward 용 서블릿
	// forward - 특정 서블릿에 대한 요청르 다른 서블릿이나 JSP 로 넘겨준다.
	// 상대방에게 페이지 주소를 숨길 때 사용할 수 있으며, redirect 보다 성능이 좋다.
	// URL 주소는 바뀌지 않으며, 내부에서만 접근이 가능하다.
	
	// 이 때 사용하는 객체는 RequestDispatcher 객체이다.
	
	// 이동되는 페이지로 값을 넘기려면 request 객체의 setAttribute()메서드로 데이터를 셋팅하여 보내고.
	// 받는쪽에서는 getAttribute()메서드로 데이터를 읽어온다.
	// 형식) setAttribute("key값", "value값);		// key값은 문자열, value 값은 모든 데이터를 다 사용할 수 있다.
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		req.setAttribute("tel", "010-6456-4563");	// Object 타입으로 넘어간다.
