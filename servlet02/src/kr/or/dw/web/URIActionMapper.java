...
// uri_mapping.properties 파일에 설정된 uri에 맞는 Action 객체를 가져와서 인스턴스를 생성해서 반환하는 역할을 수행하는 클래스
public class URIActionMapper {
	
	
	// properties의 내용을 읽어와서 uri는 키값, action 객체명은 value 값으로 해서 
	// 저장될 Map 객체 생성
	private static Map<String, String> actionMap = new HashMap<>();
	
	static {
		
			// properties 파일을 읽어와서 Bundle 객체를 생성한다.
